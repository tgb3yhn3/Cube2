...
windowHeightMultiple = 16    #視窗高度 = standardPixelSize * windowHeightMultiple
...
def web_to_raspberry(ans):
    #ans=''
    
        #print(ajaxRecognize.ansstr)
        
    #ans=ajaxRecognize.ansstr
    if(ans!=''):
        print('change!')
        for i in range(6):
            for row in range(3):
...
windowHeight = standardPixelSize*windowHeightMultiple
...
functionArea                = tk.Frame(window, width = standardPixelSize*windowWidthMultiple, height = standardPixelSize*13,bg=functionBackgroundColor)      #功能區
...
setColorByCameraButtonFrame.columnconfigure(0, weight = 1)
setColorByCameraButtonFrame.grid_propagate(0)
setColorByCameraButton = tk.Button(setColorByCameraButtonFrame,cursor="hand2",font = fontstyle1,bg = 'lightblue',text='讀取上次辨識資料',anchor="center",command=loadRecognitionDataFromJson)
setColorByCameraButton.bind("<Enter>",lambda event:mouseEnterFunctionButton(event,setColorByCameraButton))     
setColorByCameraButton.bind("<Leave>",lambda event:mouseLeaveFunctionButton(event,setColorByCameraButton))    
setColorByCameraButton.grid(sticky = "NSWE")
setColorByCameraButtonFrame.grid(row = 2, column = 3)
...
resetButton = tk.Button(resetButtonFrame,cursor="hand2",font = fontstyle1,bg = 'lightyellow',text='重設魔方顏色',anchor="center",command = lambda : resetSixFaceColor())
...
resetButtonFrame.grid(row=3,column=3)
...
startButtonFrame.grid(row=5,column=0,columnspan=5)
...
spaceAtBottomOfFunctionArea2 = tk.Frame(functionArea, width = standardPixelSize*windowWidthMultiple, height = standardPixelSize*0.5,bg=functionBackgroundColor)    #空白
spaceAtBottomOfFunctionArea2.grid(row=6,column=0,columnspan=5)
window.mainloop()
